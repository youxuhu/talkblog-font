...
<template>
  <main class="profile-page">
    ...
    <div class="profile-grid">
      ...
      <px-card stamp class="profile-card">
        <template #prepend>
          <px-icon :icon="themeStore.isDark ? 'moon-solid' : 'sun-solid'" size="26" color="#7c4dff" />
        </template>
        <template #header>
          <px-text tag="h2" size="16">主题设置</px-text>
        </template>

        <div class="theme-section">
          <div class="theme-info">
            <px-text size="14">当前模式：{{ themeStore.isDark ? '暗色模式' : '亮色模式' }}</px-text>
            <px-text size="12" type="secondary">切换应用的明暗主题</px-text>
          </div>
          <px-button
          ...
          >
            <template #prepend>
              <px-icon :icon="themeStore.isDark ? 'sun-solid' : 'moon-solid'" size="16" />
            </template>
